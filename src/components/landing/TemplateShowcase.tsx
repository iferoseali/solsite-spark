      ...
      <div className="container px-4 sm:px-6 lg:px-8">
        <div className="text-center mb-16">
          ...
          <p className="text-muted-foreground text-lg max-w-2xl mx-auto">
            6 layouts × 5 personalities. Find the perfect match for your coin's vibe.
          </p>
        </div>
        ...
        <div className="flex flex-wrap justify-center gap-3 mb-12">
          {personalities.map((p, i) => (
            <div
              key={i}
              className={`px-4 py-2 rounded-full bg-gradient-to-r ${p.color} text-primary-foreground font-medium text-sm flex items-center gap-2 hover:scale-105 transition-transform cursor-pointer`}
            >
              <span>{p.emoji}</span>
              <span>{p.name}</span>
            </div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4 mb-12">
          {layouts.map((layout, i) => (
            <div
              key={i}
              className="aspect-[3/4] rounded-xl glass overflow-hidden group cursor-pointer hover:border-primary/50 transition-all duration-300"
            >
