          ...
          {thumbnailLoaded && thumbnailHtml ? (
            <div className="w-full h-full relative overflow-hidden">
              <div className="absolute inset-0 origin-top-left" style={{ transform: "scale(0.15)", width: "667%", height: "667%" }}>
                <iframe srcDoc={thumbnailHtml} className="w-full h-full border-0 pointer-events-none" title={`Thumbnail: ${template.name}`} sandbox="allow-scripts allow-same-origin" loading="lazy" />
              </div>
            </div>
          ) : (
